export function shootingsByState() {
...
d3.csv("MassShootingsDatabase_1982_2022.csv").then(function (data) {
    ...
    data.sort((a,b) => d3.ascending(
        a.Location.slice(a.Location.indexOf(',')).replaceAll(',', ''),
        b.Location.slice(b.Location.indexOf(',')).replaceAll(',', '')
    ));
    ...
    bars.selectAll('.g-bars').append('text')
        ...
        .text(function (d) {
            var locationString = d.data.Location.slice(d.data.Location.indexOf(','));
